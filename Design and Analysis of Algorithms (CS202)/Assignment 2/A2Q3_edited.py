...
class MinCoin:
    def __init__(self, n, p):
        ...
        self.plan = [p]
    def add_plan(self, p):
        if p not in self.plan: # avoid adding duplicate plans
            self.plan.append(p)

# # parameters for testing that failed for min_coin_with_plan initially
denom  = [(3, 2), (5, 1),(6, 1)]
m = len(denom)
n = 12
...
for j in range(1, n+1): # for each amount of cents
    for i in range(m): # for each denomination
            if j >= denom[i][0] and min_coin_with_plan[j - denom[i][0]] is not None: # is it possible to use denom[i][0]? if yes, we can make up j cents by putting 1 coin of denom[i][0] and the min coin solution for j-denom[i][0]
                for p in min_coin_with_plan[j - denom[i][0]].plan: # for each possible plan of making j-denom[i][0]
                    if p[i] + 1 <= denom[i][1]: # check if the supply of denom[i][0] is enough to add one more coin
                        if min_coin_with_plan[j] is None or min_coin_with_plan[j - denom[i][0]].num_coin + 1< min_coin_with_plan[j].num_coin: # must check if there is enough supply of denom[i][0]
                            p_new = p.copy()
                            p_new[i] += 1 # increment the value of number of coins of denom[i][0] in the plan
                            min_coin_with_plan[j] = MinCoin(min_coin_with_plan[j - 1 * denom[i][0]].num_coin + 1, p_new)
...
# may print out and check the result of min_coin_with_plan
print("Min coin with plan output:")
...
# print out and check result of min_coin_num_only
print("Min coin number only output:")
# only print out the last row of min_coin_num_only if not infinity
for i in range(1, n+1):
    if min_coin_num_only[m][i] != float('inf'):
        print(i, min_coin_num_only[m][i])

all_same = True
for i in range(1, n+1):
    if min_coin_with_plan[i] is not None: # added this line as without it it was throwing an error
